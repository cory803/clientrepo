...
import org.chaos.client.accounts.Account;
...
import org.chaos.client.graphics.rsinterface.SummoningInterfaceData;
	...
	static void unpack(Archive streamLoader, TextDrawingArea textDrawingAreas[], Archive streamLoader_1, RSFontSystem[] newFontSystem) {
		...
		while (stream.position < stream.buffer.length) {
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.getString().replaceAll("RuneScape", "Chaos");
